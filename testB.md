...
1.	面向对象的程序设计具有封装、继承、多态性三大特点。
2.	算术运算符都可以重载为友元或普通函数，所有运算符重载函数都不能有默认参数。
3.	用公有派生类对象 `obj` 的地址初始化基类的指针 `p` ，则 `p` 和 `&obj` 在数值上可能不同。
...
（6）已引入头文件 `iostream` ，并且已使用命名空间 `std` 。不需要写 `main` 函数。

>**附** 指针成员访问运算符重载函数原型为 `T *operator ->()` ，返回值为可以进行指针成员访问的指针。 
